...
export async function POST(req: NextRequest) {
  const { name, email, title, description, platform } = await req.json();
  ...
  try {
    await resend.emails.send({
      ...
      to: ADMIN_EMAIL,
      replyTo: email,
      subject: `[Bug Report] ${title} — ${platform || 'Unknown platform'}`,
      html: `
        <div style="font-family:system-ui,sans-serif;max-width:560px;margin:auto;color:#1a1a18">
          <h2 style="font-size:20px;margin:0 0 20px">🐛 New bug report</h2>
          <table style="width:100%;border-collapse:collapse;font-size:15px">
            ...
            <tr><td style="padding:8px 0;color:#888;vertical-align:top">Platform</td><td style="padding:8px 0">${platform || 'Not specified'}</td></tr>
            <tr><td style="padding:8px 0;color:#888;vertical-align:top">Title</td><td style="padding:8px 0;font-weight:600">${title}</td></tr>
